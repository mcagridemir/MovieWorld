...
extension Services: TargetType {
    ...
    var path: String {
        ...
        case let .Get(UrlPrefix, page):
            if let page = page {
                return "\(version)movie/\(UrlPrefix ?? "")?api_key=\(Globals.shared.apiKey)&page=\(page)"
            }
            return "\(version)movie/\(UrlPrefix ?? "")?api_key=\(Globals.shared.apiKey)"
        }
    }
